...
from spider.diagnostics.shared_event_latent_tau import _robust_std_from_residuals


@torch.no_grad()
def _corr_error_delta_for_rows(
    *,
    state,
    II_b: torch.Tensor,
    YY_b: torch.Tensor,
    ...
    """
    Compute the corr_error nuisance correction delta_corr for a batch of rows.

    Model (per row):
      - station-basis mode:
          delta_corr = W[sta]^T ( b[e2,phase] - b[e1,phase] )
      - per-station coefficients mode (station_basis.enabled=false):
          delta_corr = (b[e2,sta,phase] - b[e1,sta,phase])

    Returns:
      delta_corr: (B,) float32 on state.device, or None if corr_error is disabled/missing.
    """
    try:
        if not bool(getattr(state, "params", {}).get("_corr_error_enabled", False)):
            return None
    except Exception:
        return None
    ...
    try:
        ...
        if not (isinstance(b, torch.Tensor) and isinstance(sta_b, torch.Tensor)):
            return None
        if II_b.ndim != 2 or int(II_b.shape[1]) < 2:
            return None
        if YY_b.ndim != 2 or int(YY_b.shape[1]) < 5:
            return None
        # b: (Ne, R, 2); W: (S, R) if present
        ...
        if isinstance(W, torch.Tensor):
            Wr = W.index_select(0, sta_b.to(torch.int64)).to(torch.float32)  # (B,R)
...
@torch.no_grad()
def _slowness_re_holdout_calibration(
    *,
    state,
    n_rows: int,
    seed: int,
    ...
    try:
        mode = str(state.params.get("_slowness_re_mode", "scalar_sep")).strip().lower()
    except Exception:
        mode = "scalar_sep"
    if mode not in {"scalar_sep", "component_station"}:
        return None
    ...
    try:
        mode_res = str(residuals_at).strip().lower()
    except Exception:
        mode_res = "map"
    if mode_res not in {"map", "initial"}:
        mode_res = "map"
    dX_use = state.dX_src
    if mode_res == "initial":
        dX_use = torch.zeros_like(state.dX_src, device=state.dX_src.device)
    ...
    def _sigma_from_quantile(r: np.ndarray, q: float) -> float:
        if r.size == 0:
            return float("nan")
        rq = np.quantile(np.abs(r), q)
        return float(rq / 0.6744897501960817)  # MAD->sigma for normal

    def _parse_sigma_override(v) -> tuple[float, float] | None:
        try:
            if v is None:
                return None
    ...
    def _robust_std_np(x: np.ndarray) -> float:
    ...
    def _collect(rows: np.ndarray):
        ...
        for i0 in range(0, int(rows.size), batch_size):
            ...
            ph = y[:, 4].detach()
            is_s = (ph > 0.5)
            x1 = (state.X_src[idx[:, 0]] + dX_use[idx[:, 0]])[:, :3]
            x2 = (state.X_src[idx[:, 1]] + dX_use[idx[:, 1]])[:, :3]
            g = torch.linalg.norm(x1 - x2, dim=1).clamp_min(1e-6)
            if sep_cap > 0.0 and np.isfinite(sep_cap):
                g = g.clamp_max(float(sep_cap))
            finite = torch.isfinite(r) & torch.isfinite(g)
            if not bool(finite.any()):
                continue
            r = r[finite]
            g = g[finite]
            is_s = is_s[finite]
            rP.append(r[~is_s].to("cpu").numpy())
            gP.append(g[~is_s].to("cpu").numpy())
            rS.append(r[is_s].to("cpu").numpy())
            gS.append(g[is_s].to("cpu").numpy())
        rP = np.concatenate(rP, axis=0) if len(rP) > 0 else np.zeros((0,), dtype=np.float64)
        gP = np.concatenate(gP, axis=0) if len(gP) > 0 else np.zeros((0,), dtype=np.float64)
        rS = np.concatenate(rS, axis=0) if len(rS) > 0 else np.zeros((0,), dtype=np.float64)
        gS = np.concatenate(gS, axis=0) if len(gS) > 0 else np.zeros((0,), dtype=np.float64)
        return rP, gP, rS, gS

    def _collect_component_station(rows: np.ndarray):
        rP, gP, cP, kP = [], [], [], []
        rS, gS, cS, kS = [], [], [], []
        sta_idx_all = getattr(state, "row_station_index", None)
        if not isinstance(sta_idx_all, torch.Tensor):
            warn("slowness_re component_station holdout: missing station index", section="DIAG")
            return None
        cluster_ids = getattr(state, "cluster_ids", None)
        if not isinstance(cluster_ids, torch.Tensor):
            cluster_ids = state.params.get("_shared_event_re_cluster_ids", None)
        if not isinstance(cluster_ids, torch.Tensor):
            warn("slowness_re component_station holdout: missing cluster ids", section="DIAG")
            return None
        for i0 in range(0, int(rows.size), batch_size):
            ii = rows[i0 : i0 + batch_size]
            idx = state.II.index_select(0, torch.from_numpy(ii).to(device=state.device, dtype=torch.int64))
            ...
            c1 = cluster_ids.index_select(0, e1)
            c2 = cluster_ids.index_select(0, e2)
            x1 = (state.X_src[e1] + dX_use[e1])[:, :3]
            x2 = (state.X_src[e2] + dX_use[e2])[:, :3]
            g = torch.linalg.norm(x1 - x2, dim=1).clamp_min(1e-6)
            if sep_cap > 0.0 and np.isfinite(sep_cap):
                g = g.clamp_max(float(sep_cap))
            finite = torch.isfinite(r) & torch.isfinite(g)
            same = (c1 == c2)
            finite = finite & same & (sta >= 0) & (c1 >= 0)
            if not bool(finite.any()):
                continue
            r = r[finite]
            g = g[finite]
            ...
            c1 = c1[finite]
            r_cpu = r.to("cpu").numpy().astype(np.float64, copy=False)
            g_cpu = g.to("cpu").numpy().astype(np.float64, copy=False)
            sta_cpu = sta.to("cpu").numpy().astype(np.int64, copy=False)
            comp_cpu = c1.to("cpu").numpy().astype(np.int64, copy=False)
            rP.append(r_cpu[~is_s.cpu().numpy()])
            gP.append(g_cpu[~is_s.cpu().numpy()])
            cP.append(comp_cpu[~is_s.cpu().numpy()])
            kP.append(sta_cpu[~is_s.cpu().numpy()])
            rS.append(r_cpu[is_s.cpu().numpy()])
            gS.append(g_cpu[is_s.cpu().numpy()])
            cS.append(comp_cpu[is_s.cpu().numpy()])
            kS.append(sta_cpu[is_s.cpu().numpy()])
        rP = np.concatenate(rP, axis=0) if len(rP) > 0 else np.zeros((0,), dtype=np.float64)
        gP = np.concatenate(gP, axis=0) if len(gP) > 0 else np.zeros((0,), dtype=np.float64)
        cP = np.concatenate(cP, axis=0) if len(cP) > 0 else np.zeros((0,), dtype=np.int64)
        kP = np.concatenate(kP, axis=0) if len(kP) > 0 else np.zeros((0,), dtype=np.int64)
        rS = np.concatenate(rS, axis=0) if len(rS) > 0 else np.zeros((0,), dtype=np.float64)
        gS = np.concatenate(gS, axis=0) if len(gS) > 0 else np.zeros((0,), dtype=np.float64)
        cS = np.concatenate(cS, axis=0) if len(cS) > 0 else np.zeros((0,), dtype=np.int64)
        kS = np.concatenate(kS, axis=0) if len(kS) > 0 else np.zeros((0,), dtype=np.int64)
        return rP, gP, cP, kP, rS, gS, cS, kS

    if mode == "component_station":
        tr = _collect_component_station(rows_train)
        te = _collect_component_station(rows_test)
        if tr is None or te is None:
            return None
        rP_tr, gP_tr, cP_tr, kP_tr, rS_tr, gS_tr, cS_tr, kS_tr = tr
        rP_te, gP_te, cP_te, kP_te, rS_te, gS_te, cS_te, kS_te = te

        def _fit_phase(r: np.ndarray, g: np.ndarray, comp: np.ndarray, sta: np.ndarray):
            if r.size == 0 or g.size == 0:
                return None
            g = np.asarray(g, dtype=np.float64)
            r = np.asarray(r, dtype=np.float64)
            comp = np.asarray(comp, dtype=np.int64)
            sta = np.asarray(sta, dtype=np.int64)
            mask = np.isfinite(r) & np.isfinite(g) & (g > 0)
            if not np.any(mask):
                return None
            r = r[mask]
            g = g[mask]
            ...
            y = r / g
            w = g * g
            n_comp = int(comp.max() + 1) if comp.size > 0 else 0
            n_sta = int(sta.max() + 1) if sta.size > 0 else 0
            if n_comp <= 0 or n_sta <= 0:
                return None
            s_c = np.zeros((n_comp,), dtype=np.float64)
            a_k = np.zeros((n_sta,), dtype=np.float64)
            for _ in range(5):
                y_minus_a = y - a_k[sta]
                sum_w_c = np.bincount(comp, weights=w, minlength=n_comp)
                sum_wy_c = np.bincount(comp, weights=w * y_minus_a, minlength=n_comp)
                valid_c = sum_w_c > 0
                s_c = np.zeros_like(s_c)
                s_c[valid_c] = sum_wy_c[valid_c] / sum_w_c[valid_c]
                # Center s_c to zero mean (weighted) and absorb into a_k.
                if np.any(valid_c):
                    mean_s = float(np.sum(s_c[valid_c] * sum_w_c[valid_c]) / np.sum(sum_w_c[valid_c]))
                    s_c = s_c - mean_s
                    a_k = a_k + mean_s
                y_minus_s = y - s_c[comp]
                sum_w_k = np.bincount(sta, weights=w, minlength=n_sta)
                sum_wy_k = np.bincount(sta, weights=w * y_minus_s, minlength=n_sta)
                valid_k = sum_w_k > 0
                a_k = np.zeros_like(a_k)
                a_k[valid_k] = sum_wy_k[valid_k] / sum_w_k[valid_k]
            pred = g * (s_c[comp] + a_k[sta])
            eps = r - pred
            sigma_est = _sigma_from_quantile(eps, float(sigma_quantile))
            if not np.isfinite(sigma_est):
                sigma_est = _robust_std_np(eps)
            tau_comp = _robust_std_np(s_c)
            tau_sta = _robust_std_np(a_k)
            return {
                "s_c": s_c,
                "a_k": a_k,
                "sigma": float(sigma_est) if np.isfinite(sigma_est) else float("nan"),
                "tau_comp": float(tau_comp),
                "tau_sta": float(tau_sta),
            }

        fitP = _fit_phase(rP_tr, gP_tr, cP_tr, kP_tr)
        fitS = _fit_phase(rS_tr, gS_tr, cS_tr, kS_tr)
        if fitP is None or fitS is None:
            return None
        sigma_fix = _parse_sigma_override(sigma_override)
        if sigma_fix is not None:
            fitP["sigma"] = float(sigma_fix[0])
            fitS["sigma"] = float(sigma_fix[1])

        def _z_stats(r: np.ndarray, g: np.ndarray, comp: np.ndarray, sta: np.ndarray, fit: dict) -> dict:
            if r.size == 0 or g.size == 0:
                return {"std": float("nan"), "n": int(r.size)}
            s_c = fit["s_c"]
            a_k = fit["a_k"]
            comp = np.asarray(comp, dtype=np.int64)
            sta = np.asarray(sta, dtype=np.int64)
            sc = np.zeros_like(comp, dtype=np.float64)
            ak = np.zeros_like(sta, dtype=np.float64)
            mask_c = comp < int(s_c.size)
            mask_k = sta < int(a_k.size)
            sc[mask_c] = s_c[comp[mask_c]]
            ak[mask_k] = a_k[sta[mask_k]]
            pred = g * (sc + ak)
            eps = r - pred
            sigma = float(fit.get("sigma", float("nan")))
            if not (np.isfinite(sigma) and sigma > 0.0):
                return {"std": float("nan"), "n": int(r.size)}
            z = eps / np.clip(sigma, 1e-12, np.inf)
            return {"std": float(np.std(z)), "n": int(z.size)}

        zP = _z_stats(rP_te, gP_te, cP_te, kP_te, fitP)
        zS = _z_stats(rS_te, gS_te, cS_te, kS_te, fitS)

        tau_units = str(state.params.get("_slowness_re_tau_units", "abs")).strip().lower()
        tau_comp_p_out = float(fitP["tau_comp"])
        tau_comp_s_out = float(fitS["tau_comp"])
        tau_sta_p_out = float(fitP["tau_sta"])
        tau_sta_s_out = float(fitS["tau_sta"])
        zP_std = float(zP.get("std", float("nan")))
        zS_std = float(zS.get("std", float("nan")))
        scale_p = float(zP_std) if np.isfinite(zP_std) and zP_std > 0.0 else 1.0
        scale_s = float(zS_std) if np.isfinite(zS_std) and zS_std > 0.0 else 1.0
        if tau_units == "vel_frac":
            try:
                ...
                vp = float(np.median(vp_arr)) if vp_arr.size > 0 else 6.0
                vs = float(np.median(vs_arr)) if vs_arr.size > 0 else 3.5
            except Exception:
                vp = 6.0
                vs = 3.5
            tau_comp_p_out = float(tau_comp_p_out) * float(vp)
            tau_comp_s_out = float(tau_comp_s_out) * float(vs)
            tau_sta_p_out = float(tau_sta_p_out) * float(vp)
            tau_sta_s_out = float(tau_sta_s_out) * float(vs)

        tau_comp_p_scaled = float(tau_comp_p_out) * float(scale_p)
        tau_comp_s_scaled = float(tau_comp_s_out) * float(scale_s)
        tau_sta_p_scaled = float(tau_sta_p_out) * float(scale_p)
        tau_sta_s_scaled = float(tau_sta_s_out) * float(scale_s)

        return {
            "mode": "component_station",
            "tau_comp_p_abs": float(fitP["tau_comp"]),
            "tau_comp_s_abs": float(fitS["tau_comp"]),
            "tau_station_p_abs": float(fitP["tau_sta"]),
            "tau_station_s_abs": float(fitS["tau_sta"]),
            "tau_units": str(tau_units),
            "tau_comp_p": float(tau_comp_p_out),
            "tau_comp_s": float(tau_comp_s_out),
            "tau_station_p": float(tau_sta_p_out),
            "tau_station_s": float(tau_sta_s_out),
            "tau_comp_p_scaled": float(tau_comp_p_scaled),
            "tau_comp_s_scaled": float(tau_comp_s_scaled),
            "tau_station_p_scaled": float(tau_sta_p_scaled),
            "tau_station_s_scaled": float(tau_sta_s_scaled),
            "sigma_p_est": float(fitP["sigma"]),
            "sigma_s_est": float(fitS["sigma"]),
            "z_p": zP,
            "z_s": zS,
            "n_train_p": int(rP_tr.size),
            "n_train_s": int(rS_tr.size),
            "n_test_p": int(rP_te.size),
            "n_test_s": int(rS_te.size),
            "sep_cap_km": float(sep_cap),
        }

    rP_tr, gP_tr, rS_tr, gS_tr = _collect(rows_train)
    rP_te, gP_te, rS_te, gS_te = _collect(rows_test)

    sigma_fix = _parse_sigma_override(sigma_override)
    if sigma_fix is not None:
        sigma_p_est, sigma_s_est = float(sigma_fix[0]), float(sigma_fix[1])
    else:
        sigma_q = float(sigma_quantile)
        sigma_p_est = _sigma_from_quantile(rP_tr, sigma_q)
        if not np.isfinite(sigma_p_est):
            sigma_p_est = _sigma_from_quantile(rP_te, sigma_q)
        sigma_s_est = _sigma_from_quantile(rS_tr, sigma_q)
        if not np.isfinite(sigma_s_est):
            sigma_s_est = _sigma_from_quantile(rS_te, sigma_q)

    def _tau_from_train(r: np.ndarray, g: np.ndarray, sigma: float) -> float:
        if r.size == 0 or g.size == 0 or (not np.isfinite(sigma)):
            return float("nan")
        g2 = g * g
        mask = g2 > 0
        if not np.any(mask):
            return float("nan")
        r2 = r * r - sigma * sigma
        v = r2[mask] / (2.0 * g2[mask])
        v = v[np.isfinite(v) & (v > 0.0)]
        if v.size == 0:
            return 0.0
        return float(np.sqrt(np.median(v)))

    tau_p_abs = _tau_from_train(rP_tr, gP_tr, sigma_p_est)
    tau_s_abs = _tau_from_train(rS_tr, gS_tr, sigma_s_est)

    def _z_stats(r: np.ndarray, g: np.ndarray, sigma: float, tau: float) -> dict:
        if r.size == 0 or g.size == 0 or (not np.isfinite(sigma)):
            return {"std": float("nan"), "n": int(r.size)}
        denom = np.sqrt(sigma * sigma + 2.0 * (tau * tau) * (g * g))
        z = r / np.clip(denom, 1e-12, np.inf)
        return {"std": float(np.std(z)), "n": int(z.size)}

    zP = _z_stats(rP_te, gP_te, sigma_p_est, tau_p_abs)
    zS = _z_stats(rS_te, gS_te, sigma_s_est, tau_s_abs)

    # Convert tau to requested units for config hint.
    tau_units = str(state.params.get("_slowness_re_tau_units", "abs")).strip().lower()
    tau_p_out = tau_p_abs
    tau_s_out = tau_s_abs
    if tau_units == "vel_frac":
        try:
            ...
            vp = 6.0
            vs = 3.5
        tau_p_out = float(tau_p_abs) * float(vp)
        tau_s_out = float(tau_s_abs) * float(vs)

    return {
        "tau_p_abs": float(tau_p_abs),
        "tau_s_abs": float(tau_s_abs),
        "tau_units": str(tau_units),
        "tau_p": float(tau_p_out) if np.isfinite(tau_p_out) else float("nan"),
        "tau_s": float(tau_s_out) if np.isfinite(tau_s_out) else float("nan"),
        "sigma_p_est": float(sigma_p_est) if np.isfinite(sigma_p_est) else float("nan"),
        "sigma_s_est": float(sigma_s_est) if np.isfinite(sigma_s_est) else float("nan"),
        "z_p": zP,
        "z_s": zS,
        "n_train_p": int(rP_tr.size),
        "n_train_s": int(rS_tr.size),
...
def _maybe_shared_event_legcorr2d(*, state, plot_dir: str) -> None:
    ...
    for label in sorted(labels):
        ...
        for tag in tags:
            ...
            print(f"Shared-event legcorr2d -> {out_png}", flush=True)
    ...
    if not aggregate_phases:
        for tag in tags:
            ...
            print(f"Shared-event legcorr2d P/S -> {out_png}", flush=True)
...
def analyze_resid_from_bundle(
    *,
        ...
        if _diag_enabled(cfg, default=False):
            ...
            if make_plots and hist_enable and (residuals_at == "initial" or not hist_initial_only):
                ...
                try:
                    ...
                    print(f"Residual histogram -> {str(plot_dir)}", flush=True)
                except Exception as e:
    ...
    # Optional: estimate hierarchical shared_event_re scales from MAP residuals.
    try:
        cfg = _get_diag_cfg(state, "shared_event_re_hier_tau_estimate")
        if _diag_enabled(cfg, default=False):
            est = estimate_shared_event_re_hier_tau_s(
                state=state,
                n_rows=int(cfg.get("n_rows", 200_000)),
                seed=int(cfg.get("seed", 0)),
                batch_size=int(cfg.get("batch_size", 50_000)),
                residuals_at=str(cfg.get("residuals_at", "map")),
                sigma_quantile=float(cfg.get("sigma_quantile", 0.2)),
                min_events_per_cluster=int(cfg.get("min_events_per_cluster", 1)),
            )
            if isinstance(est, dict):
                print(
                    "Shared-event-RE hierarchical tau estimate (analyze-resid): "
                    f"tau_event_p≈{est['tau_event_p']:.4f}s tau_event_s≈{est['tau_event_s']:.4f}s "
                    f"tau_cluster_p≈{est['tau_cluster_p']:.4f}s tau_cluster_s≈{est['tau_cluster_s']:.4f}s "
                    f"sigma_p_est≈{est['sigma_p_est']:.4f}s sigma_s_est≈{est['sigma_s_est']:.4f}s",
                    flush=True,
                )
                try:
                    out_cfg = cfg.get("outfile", None)
                    if isinstance(out_cfg, str) and out_cfg:
                        ...
                        out_path = os.path.join(str(plot_dir), "shared_event_re_hier_tau_estimate.json")
                            ...
                            "config_hint": {
                                "model": {
                                    "likelihood": {
                                        "shared_event_re": {
                                            "hierarchical": True,
                                            "tau_event_s": [float(est["tau_event_p"]), float(est["tau_event_s"])],
                                            "tau_cluster_s": [float(est["tau_cluster_p"]), float(est["tau_cluster_s"])],
                                        }
                                    }
                                }
                            },
                    ...
                    print(f"Shared-event-RE hier tau estimate -> {out_path}", flush=True)
                except Exception as e:
                    warn(f"Could not write shared_event_re hier tau estimate JSON: {e}", section="DIAG")
    except Exception as e:
        warn(f"Shared-event-RE hier tau estimate failed: {e}", section="DIAG")

    # Optional: hold-out calibration for shared_event_re hierarchical model.
    try:
        cfg = _get_diag_cfg(state, "shared_event_re_holdout")
        if _diag_enabled(cfg, default=False):
            out = _shared_event_re_holdout_calibration(
                state=state,
                n_rows=int(cfg.get("n_rows", 200_000)),
                seed=int(cfg.get("seed", 0)),
                batch_size=int(cfg.get("batch_size", 50_000)),
            ...
            if isinstance(out, dict):
                print(
                    "Shared-event-RE HOLDOUT (train/test split): "
                    f"z_std_within P={out['z_within_p']['std']:.3g} S={out['z_within_s']['std']:.3g} "
                    f"z_std_cross P={out['z_cross_p']['std']:.3g} S={out['z_cross_s']['std']:.3g} | "
                    f"sigma_est [P,S]=[{out['sigma_p_est']:.4f},{out['sigma_s_est']:.4f}] s | "
                    f"tau_event_holdout [P,S]=[{out['tau_event_p_holdout']:.4f},{out['tau_event_s_holdout']:.4f}] s | "
                    f"tau_cluster_holdout [P,S]=[{out['tau_cluster_p_holdout']:.4f},{out['tau_cluster_s_holdout']:.4f}] s",
                    flush=True,
                )
                try:
                    out_cfg = cfg.get("outfile", None)
                    if isinstance(out_cfg, str) and out_cfg:
                        ...
                        out_path = os.path.join(str(plot_dir), "shared_event_re_holdout.json")
                    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
                    import json as _json
                    out_with_hint = dict(out)
                    out_with_hint["config_hint"] = {
                        "model": {
                            "likelihood": {
                                "shared_event_re": {
                                    "hierarchical": True,
                                    "tau_event_s": [
                                        float(out["tau_event_p_holdout"]),
                                        float(out["tau_event_s_holdout"]),
                                    ],
                                    "tau_cluster_s": [
                                        float(out["tau_cluster_p_holdout"]),
                                        float(out["tau_cluster_s_holdout"]),
                                    ],
                                }
                            }
                        }
                    }
                    _json.dump(out_with_hint, open(out_path, "w"), indent=2)
                    print(f"Shared-event-RE holdout -> {out_path}", flush=True)
                except Exception as e:
                    warn(f"Could not write shared_event_re holdout JSON: {e}", section="DIAG")
    except Exception as e:
        warn(f"Shared-event-RE holdout failed: {e}", section="DIAG")

    # Optional: estimate station-phase RE tau for shared_event_re (collapsed).
    try:
        cfg = _get_diag_cfg(state, "shared_event_re_station_phase_tau_estimate")
        if _diag_enabled(cfg, default=False):
            est = estimate_shared_event_re_station_phase_tau_s(
                state=state,
                n_rows=int(cfg.get("n_rows", 200_000)),
                seed=int(cfg.get("seed", 0)),
                batch_size=int(cfg.get("batch_size", 50_000)),
                residuals_at=str(cfg.get("residuals_at", "map")),
                sigma_quantile=float(cfg.get("sigma_quantile", 0.2)),
                min_rows_per_group=int(cfg.get("min_rows_per_group", 50)),
            )
            if isinstance(est, dict):
                print(
                    "Shared-event-RE station-phase RE (within-group variance estimate): "
                    f"tau_sp [P,S]=[{est['tau_p']:.4f},{est['tau_s']:.4f}] s | "
                    f"sigma_est [P,S]=[{est['sigma_p_est']:.4f},{est['sigma_s_est']:.4f}] s | "
                    f"groups_used [P,S]=[{est['n_groups_p']},{est['n_groups_s']}]",
                    flush=True,
                )
                try:
                    out_cfg = cfg.get("outfile", None)
                    if isinstance(out_cfg, str) and out_cfg:
                        ...
                        out_path = os.path.join(str(plot_dir), "shared_event_re_station_phase_tau_estimate.json")
                    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
                    import json as _json
                    _json.dump(
                        {
                                ...
                                "model": {
                                    "likelihood": {
                                        "shared_event_re": {
                                            "station_phase_re": {
                                                "enabled": True,
                                                "tau_s": [float(est["tau_p"]), float(est["tau_s"])],
                                            }
                                        }
                                    }
                                }
                    ...
                    print(f"Shared-event-RE station-phase tau estimate -> {out_path}", flush=True)
                except Exception as e:
                    warn(f"Could not write shared_event_re station-phase tau estimate JSON: {e}", section="DIAG")
    except Exception as e:
        warn(f"Shared-event-RE station-phase tau estimate failed: {e}", section="DIAG")

    # Optional: joint holdout for shared_event_re + station-phase RE (estimate taus together).
    try:
        cfg = _get_diag_cfg(state, "shared_event_re_station_phase_holdout")
        if _diag_enabled(cfg, default=False):
            out = _shared_event_re_station_phase_joint_holdout(
                state=state,
                n_rows=int(cfg.get("n_rows", 200_000)),
                seed=int(cfg.get("seed", 0)),
                batch_size=int(cfg.get("batch_size", 50_000)),
            ...
            if isinstance(out, dict):
                print(
                    "Shared-event-RE JOINT HOLDOUT (station-phase + event): "
                    f"z_std_within P={out['z_within_p']['std']:.3g} S={out['z_within_s']['std']:.3g} "
                    f"z_std_cross P={out['z_cross_p']['std']:.3g} S={out['z_cross_s']['std']:.3g} | "
                    f"sigma_est [P,S]=[{out['sigma_p_est']:.4f},{out['sigma_s_est']:.4f}] s | "
                    f"tau_sp [P,S]=[{out['tau_sp_p']:.4f},{out['tau_sp_s']:.4f}] s | "
                    f"tau_event_holdout [P,S]=[{out['tau_event_p_holdout']:.4f},{out['tau_event_s_holdout']:.4f}] s | "
                    f"tau_cluster_holdout [P,S]=[{out['tau_cluster_p_holdout']:.4f},{out['tau_cluster_s_holdout']:.4f}] s",
                    flush=True,
                )
                try:
                    ...
                    if isinstance(out_cfg, str) and out_cfg:
                        ...
                        out_path = os.path.join(str(plot_dir), "shared_event_re_station_phase_holdout.json")
                    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
                    import json as _json
                    out_with_hint = dict(out)
                    out_with_hint["config_hint"] = {
                        "model": {
                            "likelihood": {
                                "shared_event_re": {
                                    "hierarchical": True,
                                    "tau_event_s": [float(out["tau_event_p_holdout"]), float(out["tau_event_s_holdout"])],
                                    "tau_cluster_s": [float(out["tau_cluster_p_holdout"]), float(out["tau_cluster_s_holdout"])],
                                    "station_phase_re": {
                                        "enabled": True,
                                        "tau_s": [float(out["tau_sp_p"]), float(out["tau_sp_s"])],
                                    },
                                }
                            }
                        }
                    ...
                    _json.dump(out_with_hint, open(out_path, "w"), indent=2)
                    print(f"Shared-event-RE joint holdout -> {out_path}", flush=True)
                except Exception as e:
                    warn(f"Could not write shared_event_re joint holdout JSON: {e}", section="DIAG")
    except Exception as e:
        warn(f"Shared-event-RE joint holdout failed: {e}", section="DIAG")

    # Optional: hold-out calibration for slowness_re (scalar_sep).
    try:
        cfg = _get_diag_cfg(state, "slowness_re_holdout")
        if _diag_enabled(cfg, default=False):
            out = _slowness_re_holdout_calibration(
                state=state,
                n_rows=int(cfg.get("n_rows", 200_000)),
                seed=int(cfg.get("seed", 0)),
                batch_size=int(cfg.get("batch_size", 50_000)),
                ...
                train_frac=float(cfg.get("train_frac", 0.8)),
                sigma_quantile=float(cfg.get("sigma_quantile", 0.2)),
                sep_cap_km=float(cfg.get("sep_cap_km", 0.0)),
                sigma_override=cfg.get("sigma_override", None),
            )
            if isinstance(out, dict):
                if str(out.get("mode", "scalar_sep")) == "component_station":
                    print(
                        "Slowness-RE holdout (component_station): "
                        f"z_std P={out['z_p']['std']:.3g} S={out['z_s']['std']:.3g} "
                        f"| sigma_p_est≈{out['sigma_p_est']:.4f}s sigma_s_est≈{out['sigma_s_est']:.4f}s "
                        f"| tau_comp_p≈{out['tau_comp_p']:.4f} tau_comp_s≈{out['tau_comp_s']:.4f} "
                        f"| tau_station_p≈{out['tau_station_p']:.4f} tau_station_s≈{out['tau_station_s']:.4f} ({out.get('tau_units','abs')}) "
                        f"| scaled tau_comp_p≈{out.get('tau_comp_p_scaled', out['tau_comp_p']):.4f} "
                        f"tau_comp_s≈{out.get('tau_comp_s_scaled', out['tau_comp_s']):.4f} "
                        f"tau_station_p≈{out.get('tau_station_p_scaled', out['tau_station_p']):.4f} "
                        f"tau_station_s≈{out.get('tau_station_s_scaled', out['tau_station_s']):.4f}",
                        flush=True,
                    )
                else:
                    print(
                        "Slowness-RE holdout (analyze-resid): "
                        f"z_std P={out['z_p']['std']:.3g} S={out['z_s']['std']:.3g} "
                        f"| sigma_p_est≈{out['sigma_p_est']:.4f}s sigma_s_est≈{out['sigma_s_est']:.4f}s "
                        f"| tau_p≈{out['tau_p']:.4f} tau_s≈{out['tau_s']:.4f} ({out.get('tau_units','abs')})",
                        flush=True,
                    )
                try:
                    tau_units = str(out.get("tau_units", "abs"))
                    if str(out.get("mode", "scalar_sep")) == "component_station":
                        tau_comp_p = float(out.get("tau_comp_p_scaled", out["tau_comp_p"]))
                        tau_comp_s = float(out.get("tau_comp_s_scaled", out["tau_comp_s"]))
                        tau_sta_p = float(out.get("tau_station_p_scaled", out["tau_station_p"]))
                        tau_sta_s = float(out.get("tau_station_s_scaled", out["tau_station_s"]))
                        if tau_units == "vel_frac":
                            tau_comp_hint = {"vel_frac": [tau_comp_p, tau_comp_s]}
                            tau_sta_hint = {"vel_frac": [tau_sta_p, tau_sta_s]}
                        else:
                            tau_comp_hint = [tau_comp_p, tau_comp_s]
                            tau_sta_hint = [tau_sta_p, tau_sta_s]
                        print(
                            "Slowness-RE recommended config: "
                            f"mode=component_station tau_s={tau_comp_hint} tau_station_s={tau_sta_hint} "
                            f"sigma_p≈{out['sigma_p_est']:.4f}s sigma_s≈{out['sigma_s_est']:.4f}s "
                            f"sep_cap_km={out.get('sep_cap_km', 0.0):.3g}",
                            flush=True,
                        )
                    else:
                        if tau_units == "vel_frac":
                            tau_hint = {"vel_frac": [float(out["tau_p"]), float(out["tau_s"])]}
                        else:
                            tau_hint = [float(out["tau_p"]), float(out["tau_s"])]
                        print(
                            "Slowness-RE recommended config: "
                            f"mode=scalar_sep tau_s={tau_hint} "
                            f"sigma_p≈{out['sigma_p_est']:.4f}s sigma_s≈{out['sigma_s_est']:.4f}s "
                            f"sep_cap_km={out.get('sep_cap_km', 0.0):.3g}",
                            flush=True,
                        )
                ...
                try:
                    ...
                    if isinstance(out_cfg, str) and out_cfg:
                        ...
                        out_path = os.path.join(str(plot_dir), "slowness_re_holdout.json")
                    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
                    import json as _json
                    out_with_hint = dict(out)
                    tau_units = str(out.get("tau_units", "abs"))
                    ...
                    _json.dump(out_with_hint, open(out_path, "w"), indent=2)
                    print(f"Slowness-RE holdout -> {out_path}", flush=True)
                except Exception as e:
                    warn(f"Could not write slowness_re holdout JSON: {e}", section="DIAG")
    except Exception as e:
        warn(f"Slowness-RE holdout failed: {e}", section="DIAG")
