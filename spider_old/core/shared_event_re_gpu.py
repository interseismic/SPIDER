...
def compute_slowness_quad_gpu(
    *,
    ...
    enable_profile: bool = False,
    precomputed: Optional[dict] = None,
) -> Tuple[torch.Tensor, SlownessReGpuMetrics]:
    """
    Compute total quadratic term sum for slowness_re (scalar_sep) using GPU batched PCG.
    Returns (quad_sum, metrics).
    """
    t0 = time.perf_counter()
    if isinstance(precomputed, dict):
    ...
    if group_cap is not None and int(group_cap) >= 0:
        ...
        if ncap < int(starts.numel()):
            ...
            lengths = lengths[:ncap]
    n_groups = int(starts.numel())
    metrics = SlownessReGpuMetrics(n_groups_total=n_groups)
    if n_groups == 0 or perm.numel() == 0:
        return torch.tensor(0.0, device=idx.device, dtype=resid.dtype), metrics
    try:
        metrics.max_rows_all = int(lengths.max().item()) if lengths.numel() > 0 else 0
    ...
    if not ok:
        ...
        edge_idx = torch.arange(int(perm.numel()), device=perm.device, dtype=starts.dtype)
        edge_pos = edge_idx - starts.index_select(0, group_ids)
    sigma_e = torch.where(ph_id.index_select(0, perm) < 0.5, sigma_p, sigma_s).to(resid.dtype)
    tau_g = torch.where(ph_group < 0.5, torch.tensor(float(tau_p), device=ph_group.device), torch.tensor(float(tau_s), device=ph_group.device))
    ...
    if bool(fallback_mask.any()):
        metrics.n_groups_fallback_diag = int(fallback_mask.sum().item())
        if not fallback_to_diag:
            raise ValueError("slowness_re_gpu: oversized group encountered and fallback_to_diag=false")
        for gidx in torch.nonzero(fallback_mask, as_tuple=False).reshape(-1).tolist():
            s = int(starts[gidx].item())
            e = int(ends[gidx].item())
            if e <= s:
                continue
            r_g = resid_perm[s:e]
            sigma = sigma_e[s:e].clamp_min(1e-12)
            quad_sum = quad_sum + _CollapsedQuadNoGrad.apply(
                r_g, r_g / sigma.square().clamp_min(1e-24)
            )
    ...
    if eligible_groups.numel() > 0:
        ...
        while i0 < int(eligible_groups.numel()):
            if max_edges > 0:
                ...
                i1 = i0
                while i1 < int(eligible_groups.numel()):
                    gidx = int(eligible_groups[i1].item())
                    g_len = int(lengths[gidx].item())
                    if i1 > i0 and (edge_sum + g_len) > max_edges:
                        break
                    edge_sum += g_len
                    i1 += 1
            ...
            u_pad = torch.full((B, max_edges_in_batch), -1, device=idx.device, dtype=torch.int64)
            v_pad = torch.full((B, max_edges_in_batch), -1, device=idx.device, dtype=torch.int64)
            r_pad = torch.zeros((B, max_edges_in_batch), device=resid.device, dtype=resid.dtype)
            g_pad = torch.zeros((B, max_edges_in_batch), device=resid.device, dtype=resid.dtype)
            sigma_pad = torch.ones((B, max_edges_in_batch), device=resid.device, dtype=resid.dtype)
            n_nodes_b = n_nodes.index_select(0, grp).to(torch.int64)
            tau_b = tau_g.index_select(0, grp)

            gmap = torch.full((n_groups,), -1, device=idx.device, dtype=torch.int64)
            gmap.index_copy_(0, grp, torch.arange(B, device=idx.device, dtype=torch.int64))
            row = gmap.index_select(0, group_ids)
            col = edge_pos.to(dtype=torch.int64)
            mask = (row >= 0) & (col >= 0) & (col < int(max_edges_in_batch))
            if bool(mask.any()):
                ...
                u_pad.view(-1).index_copy_(0, flat, local_u[mask])
                v_pad.view(-1).index_copy_(0, flat, local_v[mask])
                r_pad.view(-1).index_copy_(0, flat, resid_perm[mask])
                g_pad.view(-1).index_copy_(0, flat, g_perm[mask])
                sigma_pad.view(-1).index_copy_(0, flat, sigma_e[mask])

            w_pad = (g_pad * g_pad) / sigma_pad.square().clamp_min(1e-24)
            alpha = (1.0 / (tau_b.square().clamp_min(1e-24))) + float(jitter0)
            x = _pcg_batched_weighted(
                u_pad, v_pad, r_pad, w_pad, n_nodes_b,
                alpha=alpha, max_iters=int(pcg_max_iters), tol=float(pcg_tol),
            )
            ax = _edge_ax_batched(u_pad, v_pad, x, max_nodes=int(max_nodes_in_batch))
            Hx = g_pad * ax
            u_edge = (r_pad - Hx) / sigma_pad.square().clamp_min(1e-24)
            quad_sum = quad_sum + _CollapsedQuadNoGrad.apply(r_pad, u_edge)
            i0 = i1
...
def compute_slowness_comp_station_quad_gpu(
    *,
    idx: torch.Tensor,
    resid: torch.Tensor,
    g: torch.Tensor,
    ...
    """
    Compute total quadratic term sum for slowness_re component+station model using GPU batched PCG.
    Returns (quad_sum, metrics).
    """
    t0 = time.perf_counter()
    keys = ph_id
    ...
    n_nodes = int(max(n_comp, 0) + max(n_sta, 0))
    try:
        metrics.max_rows_all = int(lengths.max().item()) if lengths.numel() > 0 else 0
        metrics.max_nodes_all = int(n_nodes)
    except Exception:
        metrics.max_rows_all = 0
        metrics.max_nodes_all = 0
    ...
    g_perm = g.index_select(0, perm)
    sigma_perm = sigma.index_select(0, perm).clamp_min(1e-12)
    comp_perm = comp_id.index_select(0, perm).to(torch.int64)
    sta_perm = sta_id.index_select(0, perm).to(torch.int64)
    ph_perm = ph_id.index_select(0, perm)
    ...
    tau_comp_g = torch.where(ph_perm.index_select(0, starts) < 0.5, torch.tensor(float(tau_comp_p), device=perm.device), torch.tensor(float(tau_comp_s), device=perm.device))
    tau_sta_g = torch.where(ph_perm.index_select(0, starts) < 0.5, torch.tensor(float(tau_sta_p), device=perm.device), torch.tensor(float(tau_sta_s), device=perm.device))
    ...
    eligible = (
        ...
        & (lengths <= int(max_rows_per_group))
        & (n_nodes <= int(max_nodes_per_group))
        & ((tau_comp_g > 0.0) | (tau_sta_g > 0.0))
    )
    too_many_rows = lengths > int(max_rows_per_group)
    too_many_nodes = n_nodes > int(max_nodes_per_group)
    tau_zero = (tau_comp_g <= 0.0) & (tau_sta_g <= 0.0)
    try:
        metrics.n_groups_rows_cap = int(too_many_rows.sum().item())
        metrics.n_groups_nodes_cap = int(too_many_nodes) * int(starts.numel())
        metrics.n_groups_tau_zero = int(tau_zero.sum().item())
    except Exception:
        pass
    ...
    if bool(fallback_mask.any()):
        metrics.n_groups_fallback_diag = int(fallback_mask.sum().item())
        if not fallback_to_diag:
            raise ValueError("slowness_re component_station: oversized group encountered and fallback_to_diag=false")
        for gidx in torch.nonzero(fallback_mask, as_tuple=False).reshape(-1).tolist():
            s = int(starts[gidx].item())
            e = int(ends[gidx].item())
            if e <= s:
                continue
            r_g = resid_perm[s:e]
            sigma_g = sigma_perm[s:e].clamp_min(1e-12)
    ...
    if eligible_groups.numel() > 0:
        ...
        while i0 < int(eligible_groups.numel()):
            ...
            if bool(mask.any()):
                flat = row[mask] * int(max_edges_in_batch) + col[mask]
                u_pad.view(-1).index_copy_(0, flat, comp_perm[mask])
                v_pad.view(-1).index_copy_(0, flat, sta_perm[mask] + int(n_comp))
                r_pad.view(-1).index_copy_(0, flat, resid_perm[mask])
                g_pad.view(-1).index_copy_(0, flat, g_perm[mask])
                sigma_pad.view(-1).index_copy_(0, flat, sigma_perm[mask])

            w_pad = (g_pad * g_pad) / sigma_pad.square().clamp_min(1e-24)
            b_edge = (g_pad * r_pad) / sigma_pad.square().clamp_min(1e-24)

            tau_comp_b = tau_comp_g.index_select(0, grp)
            tau_sta_b = tau_sta_g.index_select(0, grp)
            tau_comp_b = torch.clamp(tau_comp_b, min=1e-12)
            tau_sta_b = torch.clamp(tau_sta_b, min=1e-12)
            alpha_comp = (1.0 / (tau_comp_b * tau_comp_b)).unsqueeze(1)
            alpha_sta = (1.0 / (tau_sta_b * tau_sta_b)).unsqueeze(1)
            alpha_nodes = torch.cat(
                [
                    alpha_comp.expand(B, int(n_comp)),
                    alpha_sta.expand(B, int(n_sta)),
                ],
                dim=1,
            ).to(dtype=resid.dtype)
            if float(jitter0) > 0.0:
                alpha_nodes = alpha_nodes + float(jitter0)

            n_nodes_b = torch.full((B,), int(n_nodes), device=idx.device, dtype=torch.int64)
            x = _pcg_batched_sum_weighted(
                u_pad, v_pad, b_edge, w_pad, n_nodes_b,
                alpha_nodes=alpha_nodes, max_iters=int(pcg_max_iters), tol=float(pcg_tol),
            )
            sum_x = _edge_sum_batched(u_pad, v_pad, x)
            Hx = g_pad * sum_x
            u_edge = (r_pad - Hx) / sigma_pad.square().clamp_min(1e-24)
            quad_sum = quad_sum + _CollapsedQuadNoGrad.apply(r_pad, u_edge)
            if not (np.isfinite(metrics.pred_rms) and np.isfinite(metrics.resid_rms)):
                try:
                    mask_m = (u_pad >= 0) & (v_pad >= 0)
                    if bool(mask_m.any()):
                        r_use = r_pad[mask_m]
                        p_use = Hx[mask_m]
                        g_use = g_pad[mask_m]
                        metrics.resid_rms = float(torch.sqrt(torch.mean(r_use * r_use)).item())
                        metrics.pred_rms = float(torch.sqrt(torch.mean(p_use * p_use)).item())
                        metrics.g_rms = float(torch.sqrt(torch.mean(g_use * g_use)).item())
                except Exception:
                    pass
            i0 = i1
