...
def compute_block_fim(state: LocateState, batch_size: int = 4096, return_sparse: bool = False):
    ...
    print(f"Computing FIM (Full Dataset, {N_obs} obs, {N_events} events)...")
...
def filter_unstable_events(state: LocateState, fim_diag: torch.Tensor, threshold: float = 1e-6) -> int:
    ...
    if n_drop == 0:
        print("No unstable events found.")
        return 0
        
    print(f"Dropping {n_drop} / {n_total} events due to instability.")
    ...
    print(f"Observations reduced: {n_obs_old} -> {state.N}")
    ...
    try:
        ...
        print(f"Warning: DataFrame filtering failed: {e}")
    ...
    if found:
        print("Optimizer parameters updated and old state cleared.")
    else:
        print("Warning: Could not link new dX_src to optimizer.")
...
def analyze_fim_stability(fim_diag: torch.Tensor, threshold: float = 1e-6) -> None:
    ...
    print(f"Events with min_eigenvalue < {threshold:.1e}: {n_unstable} / {fim_diag.shape[0]}")
    ...
    if n_unstable > 0:
        ...
        for idx in worst_indices:
            ...
            print(f"  Event {idx.item()}: λ={eig_val:.3e} | Vec={v_str} (Unstable in {dom_label})")
