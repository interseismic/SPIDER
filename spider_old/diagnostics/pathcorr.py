...
def run_pathcorr_diag_end_phase1(state) -> None:
    ...
    try:
        ...
        if ge.height == 0:
            print("PathCorr: no per-event residuals available after filtering; skipping.")
            return
        ...
        if len(summaries) == 0:
            print("PathCorr: no station–phase groups yielded usable statistics.")
            return
        ...
        try:
            ...
            print(f"PathCorr: wrote per-station–phase summary to {out_path}")
        except Exception as e:
            print(f"PathCorr: failed to write CSV: {e}")
        ...
        try:
            ...
            print(f"PathCorr: groups={n_groups} median rho0={med_rho0:.3f}")
            # Top-k by rho0
        ...
        print(f"PathCorr: failed with error: {e}")
