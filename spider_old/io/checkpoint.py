...
def save_checkpoint(
    ...
    corr_error_b=None,
    slowness_re_comp_p=None,
    slowness_re_comp_s=None,
    slowness_re_station_p=None,
    slowness_re_station_s=None,
    dd_graph_re_b_p=None,
    dd_graph_re_b_s=None,
    ...
    if corr_error_b is not None:
        try:
            ...
            try:
                ...
                pass
    if slowness_re_comp_p is not None:
        try:
            checkpoint_data["slowness_re_comp_p"] = slowness_re_comp_p.detach().cpu()
        except Exception:
            pass
    if slowness_re_comp_s is not None:
        try:
            checkpoint_data["slowness_re_comp_s"] = slowness_re_comp_s.detach().cpu()
        except Exception:
            pass
    if slowness_re_station_p is not None:
        try:
            checkpoint_data["slowness_re_station_p"] = slowness_re_station_p.detach().cpu()
        except Exception:
            pass
    if slowness_re_station_s is not None:
        try:
            checkpoint_data["slowness_re_station_s"] = slowness_re_station_s.detach().cpu()
        except Exception:
            pass
    if dd_graph_re_b_p is not None:
        try:
            checkpoint_data["dd_graph_re_b_p"] = dd_graph_re_b_p.detach().cpu()
        except Exception:
            pass
    if dd_graph_re_b_s is not None:
        try:
            checkpoint_data["dd_graph_re_b_s"] = dd_graph_re_b_s.detach().cpu()
        except Exception:
            pass
...
def load_checkpoint(params, device):
    ...
    # Optional explicit slowness_re latents
    if "slowness_re_comp_p" in data:
        v = data["slowness_re_comp_p"]
        if not isinstance(v, torch.Tensor):
            v = torch.tensor(v)
        out["slowness_re_comp_p"] = v.to(device=device, dtype=torch.float32)
    if "slowness_re_comp_s" in data:
        v = data["slowness_re_comp_s"]
        if not isinstance(v, torch.Tensor):
            v = torch.tensor(v)
        out["slowness_re_comp_s"] = v.to(device=device, dtype=torch.float32)
    if "slowness_re_station_p" in data:
        v = data["slowness_re_station_p"]
        if not isinstance(v, torch.Tensor):
            v = torch.tensor(v)
        out["slowness_re_station_p"] = v.to(device=device, dtype=torch.float32)
    if "slowness_re_station_s" in data:
        v = data["slowness_re_station_s"]
        if not isinstance(v, torch.Tensor):
            v = torch.tensor(v)
        out["slowness_re_station_s"] = v.to(device=device, dtype=torch.float32)
    if "dd_graph_re_b_p" in data:
        v = data["dd_graph_re_b_p"]
        if not isinstance(v, torch.Tensor):
            v = torch.tensor(v)
        out["dd_graph_re_b_p"] = v.to(device=device, dtype=torch.float32)
    if "dd_graph_re_b_s" in data:
        v = data["dd_graph_re_b_s"]
        if not isinstance(v, torch.Tensor):
            v = torch.tensor(v)
        out["dd_graph_re_b_s"] = v.to(device=device, dtype=torch.float32)
...
def prune_checkpoints_after_phase1(params) -> Tuple[int, int]:
    ...
    for p in checkpoint_dir.glob("checkpoint*.pth"):
        ...
        try:
            os.remove(str(p))
            print(f"Pruned non-phase1 checkpoint: {p}")
            deleted += 1
        except Exception as e:
            print(f"Warning: failed to prune checkpoint {p}: {e}")
    return deleted, kept
