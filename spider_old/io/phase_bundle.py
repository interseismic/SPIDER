...
class Phase2Bundle:
    ...
    # Optional: MAP value for corr_error latent (if corr_error was enabled in Phase 1).
    corr_error_b: Optional[torch.Tensor] = None  # (Ne,R,2) float32 CPU or None
...
def save_phase2_bundle(
    *,
    ...
    dtimes: pl.DataFrame,
    dX_src: torch.Tensor,
    corr_error_b: Optional[torch.Tensor] = None,
    ...
    if corr_error_b is not None:
        try:
            payload["corr_error_b"] = corr_error_b.detach().to("cpu", dtype=torch.float32).contiguous()
        except Exception:
            pass
...
def load_phase2_bundle(*, path: str) -> Phase2Bundle:
    ...
    corr_error_b = payload.get("corr_error_b", None)
    if corr_error_b is not None and (not isinstance(corr_error_b, torch.Tensor)):
        corr_error_b = None
    return Phase2Bundle(
        params=params,
        origins0=origins0,
        ...
        corr_error_b=(corr_error_b.to(torch.float32) if isinstance(corr_error_b, torch.Tensor) else None),
