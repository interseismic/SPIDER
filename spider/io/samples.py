...
def _log(*parts, section: str = "SAMPLES", **_kwargs) -> None:
...
def save_map_locations(
    ...
    try:
        ...
        f.flush()
        f.close()
        _log(f"Wrote MAP locations to {store_path} (map_longitude/map_latitude/map_depth)")
        return True
    except Exception as e:
        _log(f"Warning: could not save MAP locations to HDF5: {e}")
        return False
...
def save_samples_periodic(
    ...
    *,
    ...
    try:
        ...
        _log(f"[Samples] Starting save: {n_samples} samples, {n_events_in} events", flush=True)
        ...
        #
        # IMPORTANT:
        # - Avoid torch kernels like torch.isfinite()/nan_to_num() during save: those can trigger
        #   hard crashes (segfault) if a prior asynchronous error occurred.
        # - Avoid converting each sample to numpy up-front (150x .numpy()); instead convert
        #   chunk-by-chunk right before writing.
        _log(f"[Samples] Validating and staging samples on CPU...", flush=True)
        samples_cpu: list[torch.Tensor] = []
        for i, s in enumerate(samples):
            if isinstance(s, torch.Tensor):
                t = s.detach()
                if t.is_cuda:
                    # Shouldn't happen (we append CPU clones), but be defensive.
                    t = t.cpu()
                # Ensure a plain, contiguous float32 CPU tensor.
                t = t.to(dtype=torch.float32).contiguous()
                samples_cpu.append(t)
            else:
                a = np.asarray(s, dtype=np.float32)
                # Convert via torch to normalize shape/contiguity consistently.
                samples_cpu.append(torch.from_numpy(np.ascontiguousarray(a, dtype=np.float32)))
        
        S_batch = len(samples_cpu)
        n_events = int(samples_cpu[0].shape[0]) if S_batch > 0 else 0
        
        if n_events != n_events_in:
             _log(f"[Samples] Warning: n_events mismatch! samples={n_events}, X_src={n_events_in}", flush=True)
        ...
        # 2. Open HDF5
        _log(f"[Samples] Opening HDF5: {store_path}", flush=True)
        ...
        _log(f"[Samples] Creating group {batch_name}", flush=True)
        ...
        # 5. Write in chunks
        _log(f"[Samples] Writing datasets in chunks of {chunk_events}...", flush=True)
        ...
        for start in range(0, n_events, chunk_events):
            ...
            # (ev_count, S_batch, 4)
            # Stack in torch on CPU, then convert once to numpy to reduce overhead and avoid many .numpy() calls.
            dX_chunk_t = torch.stack([t[start:end, :] for t in samples_cpu], dim=1)  # (ev_count, S_batch, 4)
            dX_chunk = dX_chunk_t.numpy()
            if not np.isfinite(dX_chunk).all():
                # Clean non-finite values in numpy space (no torch kernels).
                dX_chunk = np.nan_to_num(dX_chunk, nan=0.0, posinf=0.0, neginf=0.0).astype(np.float32, copy=False)
            # (ev_count, 1, 3)
        ...
        f.flush()
        f.close()
        _log(f"[Samples] Successfully wrote batch {batch_idx}", flush=True)
        return sample_count
        
    except Exception as e:
        _log(f"[Samples] Fatal error during save: {e}")
        import traceback
        traceback.print_exc()
        return sample_count
...
def read_all_samples(
    ...
    if not os.path.exists(store_path):
        _log(f"Samples store not found: {store_path}")
        return {}
    ...
    with h5py.File(store_path, 'r') as f:
        ...
        if not batches:
            ...
            if map_lon_root is not None and map_lat_root is not None and map_dep_root is not None:
                ...
                _log("No batch_* groups found; returning MAP-only samples (n_samples=1).")
            ...
            _log("No batches found in samples store")
            return {}
        ...
        if not n_events_per_batch:
            _log("No valid batches found in samples store")
            return {}
        ...
        if has_epoch and any(has_epoch) and (not all(has_epoch)):
            ...
            if legacy_mode == "drop":
                try:
                    _log(f"Warning: {msg} Dropping legacy batches (read_samples_legacy_mode='drop').")
                except Exception:
                    pass
                batches = modern_batches
            else:
                try:
                    _log(f"Warning: {msg} Keeping legacy batches (read_samples_legacy_mode='keep').")
                except Exception:
                    pass
